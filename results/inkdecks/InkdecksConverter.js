...
export default class InkdecksConverter {
    ...
    static readTournamentData = async function(fileName) {
        ...
        const tournamentsContent = await fs.readFile(fileName, { encoding: 'utf8' });
    ...
    static convertTournamentDimension = async function(options) {
        ...
        const tournamentList = await InkdecksConverter.readTournamentData(options.sourceFileName);
        ...
        tournamentList.forEach((tournament, index) => {
            console.log(`Processing tournament ${index+1} out of ${tournamentList.length}`);
        ...
        await InkdecksConverter.storeResultData(options.tournamentDimensionFileName,options.persistTournamentDimension,tournamentDimensionData);
    }
    ...
    static convertDecklistDimension = async function(options) {
        ...
        tournamentList.forEach((tournament, index) => {
            ...
            tournament.decklists.forEach((decklist, index) => {
                console.log(`Processing decklist ${index+1} out of ${tournament.decklists.length}`);
        ...
        await InkdecksConverter.storeResultData(options.decklistDimensionFileName,options.persistDecklistDimension,decklistDimensionData);
    }
    ...
    static convertCardlistFact = async function(options) {
        ...
        tournamentList.forEach((tournament, index) => {
            ...
            tournament.decklists.forEach((decklist, index) => {
                ...
                decklist.cards.forEach((card, index) => {
                    console.log(`Processing card ${index+1} out of ${decklist.cards.length}`);
        ...
        await InkdecksConverter.storeResultData(options.cardlistFactFileName,options.persistCardlistFact,cardlistFactData);
    }
}